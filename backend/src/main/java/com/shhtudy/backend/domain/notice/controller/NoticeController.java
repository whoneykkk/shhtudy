...
public class NoticeController {
    ...
    @GetMapping
    public ResponseEntity<ResponseCustom<Page<NoticeListResponseDto>>> getAllNotices(
        ...
        return ResponseEntity.ok(ResponseCustom.ok(response));
    }

    @Operation(summary = "공지 상세 조회", description = "공지 1개를 상세 조회합니다. 조회한 공지는 읽음 처리 합니다.")
    @GetMapping("/{noticeId}")
    public ResponseEntity<ResponseCustom<NoticeResponseDto>> getNoticeDetail(
            @RequestHeader("Authorization") String authorizationHeader,
    ...
    @GetMapping("/mypage")
    public ResponseEntity<ResponseCustom<MyPageNoticesResponseDto>> getUnreadNoticeForMyPage(
            @RequestHeader("Authorization") String authorizationHeader
    ) {
        String userId = firebaseAuthService.verifyIdToken(authorizationHeader.replace("Bearer ", ""));
        ...
        if (response.getNotices().isEmpty()) {
            return ResponseEntity.ok(ResponseCustom.of(response, "읽지 않은 공지가 없습니다.", HttpStatus.OK));
        }
